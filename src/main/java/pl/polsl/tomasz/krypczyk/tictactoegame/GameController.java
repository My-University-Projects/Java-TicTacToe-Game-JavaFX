...
import pl.polsl.tomasz.krypczyk.tictactoegame.model.*;
...
public class GameController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        this.currentPlayerLabel.setText(player1Name);
        try {
            this.begin();
    ...
    public void begin() throws FileNotFoundException, IOException {
        ...
        GameController.gameStatus = GameStatus.IN_GAME;
    ...
    private void createSingleplayerGame() {
        this.player1 = createPlayer(player1Name, 'O');
    ...
    private void createMultiplayerGame() {
        this.player1 = createPlayer(player1Name, 'O');
        this.player2 = createPlayer(player2Name, 'X');
    }
        ...
        if (this.roundCount < 9) {
            switch (this.actualPlayer) {
                case 1 -> {
                    ...
                    if (isMoveValid(player1, board, position)) {
                        button.setText(Character.toString(player1.getSign()));
                        if (winningConditions.isWinner(player1)) {
                    ...
                    if (player2 instanceof ComputerPlayer && didMove && roundCount < 9) {

                        while (true) {
                            int intPosition = 1 + (int) (Math.random() * 9);
                            if (board.updateBoard(player2, intPosition)) {
                                addPlayerPosition(player2, intPosition);
                                this.setCpusSign(player2.getSign(), intPosition);
                                break;
                            }
                        }
                        if (winningConditions.isWinner(player2)) {
                ...
                case 2 -> {
                    if (isMoveValid(player2, board, position)) {
     ...
     * @param position
     */
    public boolean isMoveValid(Player player, Board board, int position) {
        if (board.updateBoard(player, position)) {
    ...
    private void updateTable() {
        this.topPlayers.getTopPlayers().forEach(p -> {
            this.TopPlayersTable.getItems().add(p);
        });
    }
